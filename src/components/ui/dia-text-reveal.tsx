...
export function DiaTextReveal({
  ...
  }, [Array.isArray(text) ? text.join("\0") : text]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    playRef.current()
    return () => { stopRef.current?.(); clearTimeout(timerRef.current); hasPlayedRef.current = false }
